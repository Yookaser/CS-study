...
- 웹 기술
  - 오늘날의 웹 기술은 전통적인 계산의 경계를 확장하고 있음
    - **포털**을 통해 회사들은 자신의 내부 서버에 웹 접근을 구현
    - **네트워크 컴퓨터(thin client)**는 실질적으로 웹 기반 컴퓨팅을 이해하는 터미널을 말하며, 이들은 더 높은 보안이나 쉬운 보수가 필요한 경우 전통적 워크스테이션을 대체하여 사용
    - 휴대용 컴퓨터 또는 **무선 네트워크**나 이동전화 망에 연결되어 회사의 웹 포털을 사용할 수 있게 함
  - 네트워크의 고속 연결은 프린터, 클라이언트 PC, 서버를 갖는 자신의 네트워크를 가질 수 있게 만듬
    - **방화벽**은 이런 홈 계산 환경을 보안 침해로부터 보호하기 위해 존재(네트워크의 장치간 통신을 제한)
  - **시분할 시스템**은 대표적인 전통적 시스템으로 각 사용자에게 프로세스를 CPU에 하나씩 순환시켜 가며 실행시키지 위해 타이머와 스케줄링 알고리즘을 이용
...
- **모바일 컴퓨팅**
  - 휴대폰 스마트폰과 태블릿 컴퓨터의 컴퓨팅 환경
  - 이동 가능하고 가볍다는 물리적 특징을 공유(대신 화면 크기, 메모리 용량, 몇몇 기능을 포기함)
  - 다만, 최근에는 GPS(전역 위치 시스템), 가속도계, 자이로스코프 등을 이용해 독특한 기능을 활용하는 응용들을 설계
    - **증강현실**은 이러한 기능의 더 실용적 이용으로 현재 환경의 디스플레이 위에 정보를 겹치는 것
  - 온라인 서비스 접근을 허용하기 위해 휴대장치는 전형적으로 IEEE 표준 802.11 무선 또는 휴대 전화 데이터망을 사용
  - 현재 휴대 컴퓨팅에서 두 개의 지배적인 운영체제는 **Apple IOS**와 **Google Android**
...
- 현대 네트워크 구조
  - **서버 시스템**이 **클라이언트 시스템**이 생성한 요청을 만족시키는 배치(**클라이언트-서버 시스템**)
    - **계산-서버 시스템**
      - 클라이언트가 어떤 작업을 요청할 인터페이스를 제공
      - 서버는 해당 작업을 수행하고 결과를 클라이언트에게 돌려보냄
      - ex) 클라이언트의 데이터 요청에 응답하는 데이터베이스를 실행하고 있는 서버
    - **파일-서버 시스템**
      - 클라이언트가 파일을 생성, 갱신, 읽기 및 제거할 수 있는 인터페이스를 제공
      - ex) 웹 브라우저 클라이언트에게 파일(비디오, 콘텐츠, 웹 페이지 등)을 전달하는 웹 서버
...
- 피어 간 계산
  - 분산 시스템의 다른 구조로 이 모델에서는 클라이언트와 서버가 구별되지 않음
  - 모든 노드가 피어로 간주되고 각 피어는 서비스를 요청하는지 제공하는지에 따라 클라이언트 및 서버로 작동
  - 장점(전통적 클라이언트 서버에 비해)
    - 클라이언트 서버 시스템에서는 서버가 병목으로 작용하나 피어 간 시스템에서는 서비스가 네트워크에 분산된 여러 노드에 의해 제공될 수 있음
  - 피어 간 시스템에 참여하기 위해서는 노드는 먼저 피어 간 네트워크에 참가해야 함
    - 일단 참가하면 다른 노드에게 서비스를 제공하거나 요청할 수 있음
  - 어떤 서비스가 가능한지 결정하는 2가지 방법
    1. 노드가 네트워크에 참가할 때 네트워크 중앙 검색 서비스에 자신이 제공하는 서비스를 등록
       - 특정 서비스를 원하는 노드는 먼저 중앙 검색 서비스를 통해 어떤 노드가 서비스를 제공하는지 확인 후, 나머지 통신은 노드 간 발생
    2. 클라이언트로 동작하는 피어는 원하는 서비스를 제공하는 노드를 찾기 위해 모든 노드에게 서비스 요청 메세지를 방송
       - 이에 해당하는 노드는 메세지를 보냄
       - 이 기술을 위해서는 피어들이 다른 피어들이 제공하는 서비스를 발견하기 위한 **발견 프로토콜**이 제공되어야 함



- **피어 간 네트워킹**
  - **Napster**
    - 중앙 서버가 Napster 네트워크 상에 존재하는 모든 노드들이 가지고 있는 파일의 인덱스를 유지하고 실제 파일 전송은 피어 노드에서 일어남
  - **Gnutella**
    - 클라이언트는 파일 전송 요청을 방송하고, 이에 응할 수 있는 서버는 클라이언트에게 직접 응답



- 파일 교환의 현재
  - 파일을 교환할 때, 저작권법의 문제가 있기 때문에 해당 기술의 미래는 불투명
  - **IP상 음성(VOIP)**의 기술을 이용한 Skype는 절충형 피어 간 접근을 사용
    - 중앙형 로그인 서버를 가지며 분산형 피어를 사용하여 두 피어가 서로 통신할 수 있게 함
...
- **클라우드 컴퓨팅**
  ...
  - 클라우드 컴퓨팅의 유형
    ...
    - **SaaS(소프트웨어 서비스)**
      - 인터넷을 통해 사용 가능한 하나 이상의 응용(워드 프로세스, 스프레드 시트 등)
    - **PaaS(플랫폼 서비스)**
      - 인터넷을 통해 사용하도록 응용에 맞게 준비된 소프트웨어 스택(데이터베이스 서버 등)
    - **IaaS(하부구조 서비스)**
      - 인터넷을 통해 사용 가능한 서버나 저장장치(생산 데이터의 백업 복사본을 만들기 위한 저장장치 등)
...
- 내장형 시스템
  - 현재 가장 유행하는 컴퓨터의 형태로 자동차 엔진, 공장용 로봇 등에서 하드웨어 장치들을 감시하고 관리
    - 대부분의 시스템이 원시적이고, 따라서 운영체제도 제한된 기능을 제공
  - 대부분 **실시간 운영체제**를 수행
    - 처리기의 작동이나 데이터의 흐름에 엄격한 시간 제약이 있을 때 사용되고, 따라서 실시간 시스템은 종종 제어 장치로 사용됨
    - 제한 시간 내에 반드시 처리가 이뤄져야 함(아니면 시스템은 실패함)



## 1.11 무료 및 공개 소스 운영체제

- **무료 운영체제**와 **공개 소스 운영체제**는 컴파일된 이진 코드가 아닌 소스 코드 형식을 제공
  - 소스 코드의 무료 사용을 넘어 재배포 및 수정을 허용하는 라이선스를 받음
  - GNU/Linux가 대표적인 공개 소스 운영체제이고, Windows는 대표적인 **비공개 소스**, Apple의 macOS는 하이브리드 방식



### 1.11.1 역사

- 컴퓨팅의 초기에는 소프트웨어는 소스 코드와 함께 제공되었음
- 컴퓨터 및 소프트웨어 회사는 사용 승인된 컴퓨터 및 유료 고객에게만 제한하고자 소스 코드에서 컴파일화된 이진 파일만 릴리즈



### 1.11.2 무료 운용체제

- 자유 소프트웨어 운동(**자유 소프트웨어 재단(FSF)**)
  - 소프트웨어의 금전 거래를 반대하는 것은 아니며 다음의 4가지를 주장
...
- 인터럽트는 하드웨어가 운영체제와 상호 작용하는 주요 방법
  - 하드웨어 장치는 CPU에 신호를 보내 인터럽트를 촉발하여 일부 이벤트에 주의가 필요하다는 것을 CPU에 경고
  - 인터럽트는 인터럽트 핸들러에 의해 관리
- 컴퓨터가 프로그램을 실행하려면 프로그램이 메인 메모리에 있어야 함
  - 이 메모리는 프로세서가 직접 액세스 할 수 있는 유일한 대용량 저장소
- 메인 메모리는 일반적으로 전원을 끌때, 내용이 손실되는 휘발성 저장장치
- 비휘발성 저장장치는 메인 메모리의 확장이며 대량의 데이터를 영구적으로 보유할 수 있음
- 가장 일반적인 비휘발성 저장장치는 프로그램과 데이터를 모두 저장할 수 있는 하드디스크
- 컴퓨터 시스템의 다양한 저장장치 시스템은 속도와 비용에 따라 계층 구조로 구성할 수 있음
  - 레벨이 높을수록 비싸지만 속도가 빠름, 반대로 내려갈수록 비트당 비용은 감소하지만 액세스 시간은 증가
- 최신 컴퓨터 아키텍처는 각 CPU에 여러 컴퓨팅 코어가 포함된 다중 처리기 시스템
- CPU를 최대한 활용하기 위해 최신 운영체제는 다중 프로그래밍을 사용하여 여러 작업을 동시에 메모지에 적재할 수 있으므로 항상 CPU가 실행할 작업이 있게 보장
- 다중 태스킹은 CPU 스케줄링 알고리즘이 프로세스 간에 빠르게 전환하여 사용자에게 빠른 응답 시간을 제공하는 다중 프로그래밍의 확장
- 사용자 프로그램이 시스템의 올바른 작동을 방해하지 않도록 시스템 하드웨어에는 사용자 모드와 커널 모드 두 가지 모드가 있음
- 다양한 명령어가 특권을 가지며 커널 모드에서만 실행될 수 있음
  - ex) 터널 모드로 전환하는 명령, I/O 제거, 타이머 관리, 인터럽트 관리
- 프로세스는 운영체제의 기본 작업 단위
  - 프로세스 관리에는 프로세스 생성 및 삭제와 프로세스 간 통신 및 동기화 기법 제공이 포함
- 운영체제는 사용 중인 메모리 영역과 사용 중인 프로세스를 추적하여 메모리를 관리
  - 메모리 공간을 동적으로 할당하고 해제해야 함
- 저장장치 공간은 운영체제에서 관리
  - ex) 파일 시스템 제공, 대용량 저장장치 공간 관리 등
- 운영체제는 운영체제 및 사용자를 보호하고 안전하게 만들기 위한 여러 기법이 존재
  - 보호 조치는 컴퓨터 시스템에서 사용 가능항 자원에 대한 프로세스 또는 사용자의 액세스를 제어
- 가상화에는 컴퓨터 하드웨어를 여러 가지 실행 환경으로 추상화하는 작업이 포함
- 운영체제에서 사용되는 자료구조는 리스트, 스택, 큐, 트리, 맵 등
- 컴퓨팅은 전통적인 컴퓨팅, 모바일 컴퓨팅, 클라이언트-서버 시스템, 피어 간 시스템, 클라우드 컴퓨팅, 실시간 임베디드 시스템 등 다양한 환경에서 이뤄짐
- 무료 및 공개 소스 운영체제는 소스 코드 형식으로 제공
  - 무료 사용, 재배포, 수정이 가능
  - ex) GNU/Linux, FreeBSD, Solaris 등
